...
| TYP-007 | done | StorageStats Struct | `tests/storage_types/typ_007_storage_stats.rs` covers TYP-007 test plan (defaults, field assignment, clone/eq, Debug), plus explicit “no RocksDB” construction per acceptance. |
| TYP-008 | done | BlockStoreConfig Struct | `tests/storage_types/typ_008_config.rs` asserts TYP-008 default table + extension defaults, structural override, `Clone`, and `Debug`; `path` is the RocksDB root ([`TYP-008.md`](specs/TYP-008.md)). |
