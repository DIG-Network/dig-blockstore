...
|--------|-------------|-------------------|--------------|--------|
| KEY-001 | Hash keys MUST be raw 32-byte Bytes32 values with no prefix | Unit test: `hash_key` length, byte identity, round-trip `Bytes32`, distinct inputs | `tests/unit/key_encoding/test_key_001_hash_keys.rs` | Done |
