...
| KEY-004 | Metadata keys MUST be UTF-8 encoded strings | Unit test: `metadata_key` byte tables, variable length, no trailing NUL, distinct well-known keys, `META_*` literals, UTF-8 round-trip | `tests/test_key_004_metadata_keys.rs` | Done |
