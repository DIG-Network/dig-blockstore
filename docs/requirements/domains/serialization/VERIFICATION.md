...
|----|--------|---------|----------------------|
| SER-001 | gap | Block serialization with bincode + zstd dictionary compression; read path reverses | Unit: round-trip serialize/deserialize full block; verify compressed bytes in CF_BLOCKS; verify decompression with dictionary produces original block; verify 3-5x compression ratio on representative blocks |
