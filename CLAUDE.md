# dig-blockstore — Project Context

## What This Is

dig-blockstore — Persistent block and checkpoint storage, chain canonicality, and pruning.
...
```
gitnexus_impact({target: "BlockStore", direction: "upstream"})
...
This project is indexed by GitNexus as **dig-blockstore**. Run `npx gitnexus analyze` to build or refresh the index. Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
3. `READ gitnexus://repo/dig-blockstore/process/{processName}` — trace the full execution flow step by step
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/dig-blockstore/context` | Codebase overview, check index freshness |
| `gitnexus://repo/dig-blockstore/clusters` | All functional areas |
| `gitnexus://repo/dig-blockstore/processes` | All execution flows |
| `gitnexus://repo/dig-blockstore/process/{name}` | Step-by-step execution trace |
